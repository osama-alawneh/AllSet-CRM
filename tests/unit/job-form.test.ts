...
describe('parseJobForm', () => {
  it('parses a full form', () => {
    expect(parseJobForm(fd({
      ...
      customer_id: 3, service: 'Full house', description: '22 panes', scheduled_date: '2026-07-10', price: 240, cleaner_amount: null,
    }});
  });
  it('empty optionals become null', () => {
    expect(parseJobForm(fd({ customer_id: '3', service: 'S', description: '', scheduled_date: '', price: '' })))
      .toEqual({ ok: true, value: { customer_id: 3, service: 'S', description: null, scheduled_date: null, price: null, cleaner_amount: null } });
  });
  ...
  it('accepts a datetime-local value (date + time)', () => {
    expect(parseJobForm(fd({ customer_id: '3', service: 'x', scheduled_date: '2026-07-10T14:30' }))).toEqual({
      ok: true, value: { customer_id: 3, service: 'x', description: null, scheduled_date: '2026-07-10T14:30', price: null, cleaner_amount: null },
    });
  });
  it('still accepts a bare date (no time) for backward compatibility', () => {
    expect(parseJobForm(fd({ customer_id: '3', service: 'x', scheduled_date: '2026-07-10' }))).toEqual({
      ok: true, value: { customer_id: 3, service: 'x', description: null, scheduled_date: '2026-07-10', price: null, cleaner_amount: null },
    });
  });
