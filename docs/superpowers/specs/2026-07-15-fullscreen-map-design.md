...
- Toolbar floats: `.map-full .maptools` becomes `position: absolute; top: 10px; left: 10px; right: 10px; z-index: 15; margin: 0; padding: 8px; border-radius: 6px; background: color-mix(in srgb, var(--card) 82%, transparent); backdrop-filter: blur(4px);`. Same children (search, DotCounts, layer chips, hint); the search dropdown (`.sresults`, z-60) still overlays fine.
